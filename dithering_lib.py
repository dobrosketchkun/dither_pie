...
__all__ = [
    ...
    # Individual strategies (for advanced usage)
    'NoDitherStrategy',
    'MatrixDitherStrategy',
    'BayerDitherStrategy',
    'BlueNoiseDitherStrategy',
    'InterleavedGradientNoiseDitherStrategy',
    'ErrorDiffusionDitherStrategy',
    'OstromoukhovDitherStrategy',
    'RiemersmaDitherStrategy',
    'PolkaDotDitherStrategy',
    'WaveletDitherStrategy',
    'AdaptiveVarianceDitherStrategy',
    'PerceptualDitherStrategy',
    'HybridDitherStrategy',
    'HalftoneDitherStrategy',
    # Utility functions
    'generate_blue_noise',
]
...
class PixelizeMethod(Enum):
    """Pixelization methods available for image/video processing."""
    NONE = "none"
    REGULAR = "regular"
    NEURAL = "neural"


class PaletteSource(Enum):
    """Palette generation sources."""
...
class ErrorDiffusionKernel:
    """
    Predefined error diffusion kernels with weights and metadata.
    
    Each kernel is a dict with:
    - 'weights': List of (dx, dy, weight) tuples for error distribution
    - 'divisor': Sum of weights (for normalization, if not pre-normalized)
    - 'description': Human-readable description
    - 'rows': Number of rows the kernel spans (for documentation)
    """
    ...
    FLOYD_STEINBERG = {
        ...
        'divisor': 16,
        'description': 'Classic Floyd-Steinberg (4 neighbors)',
        'rows': 2
    }
    ...
    JJN = {
        ...
        'divisor': 48,
        'description': 'Jarvis-Judice-Ninke (12 neighbors, smooth gradients)',
        'rows': 3
    }
    ...
    STUCKI = {
        ...
        'divisor': 42,
        'description': 'Stucki (12 neighbors, photographic quality)',
        'rows': 3
    }
    ...
    BURKES = {
        ...
        'divisor': 32,
        'description': 'Burkes (7 neighbors, fast)',
        'rows': 2
    }
    ...
    ATKINSON = {
        ...
        'divisor': 8,  # Only 6/8 distributed, loses 25% error intentionally
        'description': 'Atkinson (6 neighbors, classic Mac look)',
        'rows': 3
    }
    ...
    SIERRA = {
        ...
        'divisor': 32,
        'description': 'Sierra Full (10 neighbors, high quality)',
        'rows': 3
    }
    ...
    SIERRA_TWO_ROW = {
        ...
        'divisor': 16,
        'description': 'Sierra Two-Row (8 neighbors, balanced)',
        'rows': 2
    }
    ...
    SIERRA_LITE = {
        ...
        'divisor': 4,
        'description': 'Sierra Lite (4 neighbors, fastest)',
        'rows': 2
    }
    
    @classmethod
    def get_kernel(cls, name: str) -> Dict[str, Any]:
        """Get kernel by name."""
        kernels = {
            'floyd_steinberg': cls.FLOYD_STEINBERG,
            'jjn': cls.JJN,
            'stucki': cls.STUCKI,
            'burkes': cls.BURKES,
            'atkinson': cls.ATKINSON,
            'sierra': cls.SIERRA,
            'sierra_two_row': cls.SIERRA_TWO_ROW,
            'sierra_lite': cls.SIERRA_LITE,
        }
        return kernels.get(name, cls.FLOYD_STEINBERG)
...
class BaseDitherStrategy:
    ...
    @staticmethod
    def get_parameter_info() -> Optional[Dict[str, Any]]:
        """Override to provide configurable parameters metadata."""
        return None
    
    def get_current_parameters(self) -> Dict[str, Any]:
        """Override to return current parameter values."""
        return {}
...
class BlueNoiseDitherStrategy(MatrixDitherStrategy):
    ...
    # In-memory cache for generated blue noise matrices
...
class ErrorDiffusionDitherStrategy(BaseDitherStrategy):
    ...
    Supported variants:
    - floyd_steinberg: 4 neighbors, classic and fast
    ...
    - sierra_two_row: 8 neighbors, balanced
    - sierra_lite: 4 neighbors, fast
    """
    ...
    def get_parameter_info() -> Dict[str, Any]:
        ...
        return {
            'variant': {
                'type': 'choice',
                'default': 'atkinson',
                'choices': ErrorDiffusionKernel.list_kernels(),
                'label': 'Algorithm',
    ...
    def __init__(self, variant: str = 'atkinson', serpentine: str = 'false'):
        ...
        Args:
            ...
            serpentine: Whether to use serpentine scanning ('true' or 'false')
        """
        self.variant = variant
        self.serpentine = (serpentine == 'true')
        self._kernel = ErrorDiffusionKernel.get_kernel(variant)
    ...
    def dither(self, pixels: np.ndarray, palette_arr: np.ndarray,
        ...
        for y in range(h):
            ...
            for x in x_range:
                ...
                # Distribute error using kernel weights
                for dx, dy, weight in weights:
                    # Adjust dx for serpentine scanning
                    actual_dx = dx * x_direction
                    nx = x + actual_dx
                    ny = y + dy
                    
                    if 0 <= nx < w and 0 <= ny < h:
                        work_2d[ny, nx] += err * (weight / divisor)
...
class PolkaDotDitherStrategy(BaseDitherStrategy):
    ...
    def get_current_parameters(self) -> Dict[str, Any]:
        """Returns current parameter values."""
        return {'tile_size': self.tile_size, 'gamma': self.gamma}
    ...
    def get_parameter_info() -> Dict[str, Any]:
        ...
        return {
            'wavelet': {
                ...
                'label': 'Wavelet Type',
                'description': 'Type of wavelet basis function'
            },
            'subband_quant': {
                'type': 'int',
                'default': 8,
...
class AdaptiveVarianceDitherStrategy(BaseDitherStrategy):
    """
    Adaptive dithering that checks local variance:
      - If the local variance is below a threshold => skip error diffusion
      - If it's above the threshold => do Floyd–Steinberg distribution
    """
    ...
    def get_parameter_info() -> Dict[str, Any]:
        ...
        return {
            'var_threshold': {
                ...
                'description': 'Threshold for local variance to trigger error diffusion'
            },
            'window_radius': {
                'type': 'int',
                'default': 1,
                ...
                'max': 5,
                'label': 'Window Radius',
                'description': 'Radius of window for computing local variance'
            }
        }
    ...
    def get_current_parameters(self) -> Dict[str, Any]:
        """Returns current parameter values."""
        return {'var_threshold': self.var_threshold, 'window_radius': self.window_radius}

    def dither(self, pixels: np.ndarray, palette_arr: np.ndarray,
               image_size: Tuple[int, int]) -> np.ndarray:
        h, w = image_size

        # Compute a local variance map from grayscale
        ...
        # Row-major error diffusion pass
...
class HybridDitherStrategy(BaseDitherStrategy):
    """
    A "hybrid" approach: row-major error diffusion, but only fully diffuse 
    the luminance portion of the error, while reducing the color portion.
    """
    ...
    def get_current_parameters(self) -> Dict[str, Any]:
        """Returns current parameter values."""
        return {'lum_factor': self.lum_factor, 'col_factor': self.col_factor}
    ...
    def dither(self, pixels: np.ndarray, palette_arr: np.ndarray,
        ...
        if _NUMBA_AVAILABLE:
            try:
                palette_f = palette_arr.astype(np.float32, copy=False)
                work_2d = _hybrid_numba(
                    work_2d,
                    palette_f,
                    float(self.lum_factor),
                    float(self.col_factor)
                )
                return work_2d.reshape((-1, 3))
            except Exception:
                pass
...
class OstromoukhovDitherStrategy(BaseDitherStrategy):
    ...
    def get_current_parameters(self) -> Dict[str, Any]:
        """Returns current parameter values."""
        return {'serpentine': 'true' if self.serpentine else 'false'}
...
if _NUMBA_AVAILABLE:
    @njit(cache=True)
    def _hybrid_numba(work_2d: np.ndarray,
                      palette_arr: np.ndarray,
                      lum_factor: float,
                      col_factor: float) -> np.ndarray:
        h = work_2d.shape[0]
        w = work_2d.shape[1]
        palette_len = palette_arr.shape[0]
        ...
        for y in range(h):
            for x in range(w):
                ...
                # split error into luminance and color components
                lum_err_val = 0.299 * err0 + 0.587 * err1 + 0.114 * err2
                lum0 = 0.299 * lum_err_val
                lum1 = 0.587 * lum_err_val
                lum2 = 0.114 * lum_err_val
                fe0 = lum_factor * lum0 + col_factor * (err0 - lum0)
                fe1 = lum_factor * lum1 + col_factor * (err1 - lum1)
                fe2 = lum_factor * lum2 + col_factor * (err2 - lum2)

                # Floyd-Steinberg distribution
                if x + 1 < w:
                    work_2d[y, x + 1, 0] += fe0 * (7.0 / 16.0)
                    work_2d[y, x + 1, 1] += fe1 * (7.0 / 16.0)
                    work_2d[y, x + 1, 2] += fe2 * (7.0 / 16.0)
                if y + 1 < h:
                    if x - 1 >= 0:
                        work_2d[y + 1, x - 1, 0] += fe0 * (3.0 / 16.0)
                        work_2d[y + 1, x - 1, 1] += fe1 * (3.0 / 16.0)
                        work_2d[y + 1, x - 1, 2] += fe2 * (3.0 / 16.0)
                    work_2d[y + 1, x, 0] += fe0 * (5.0 / 16.0)
                    work_2d[y + 1, x, 1] += fe1 * (5.0 / 16.0)
                    work_2d[y + 1, x, 2] += fe2 * (5.0 / 16.0)
                    if x + 1 < w:
                        work_2d[y + 1, x + 1, 0] += fe0 * (1.0 / 16.0)
                        work_2d[y + 1, x + 1, 1] += fe1 * (1.0 / 16.0)
                        work_2d[y + 1, x + 1, 2] += fe2 * (1.0 / 16.0)
...
class HalftoneDitherStrategy(BaseDitherStrategy):
    ...
    def dither(self, pixels: np.ndarray, palette_arr: np.ndarray,
        ...
        # Vectorized cell color computation
    ...
    def _generate_halftone_screen_with_cells(self, h: int, w: int) -> Tuple[np.ndarray, np.ndarray]:
        """Generate halftone screen and cell assignments."""
...
class DitherUtils:
    ...
    BAYER16x16 = np.array([
        [0.00390625, 0.50390625, 0.12890625, 0.62890625, 0.03515625, 0.53515625, 0.16015625, 0.66015625,
         0.01171875, 0.51171875, 0.13671875, 0.63671875, 0.04296875, 0.54296875, 0.16796875, 0.66796875],
        [0.75390625, 0.25390625, 0.87890625, 0.37890625, 0.78515625, 0.28515625, 0.91015625, 0.41015625,
         0.76171875, 0.26171875, 0.88671875, 0.38671875, 0.79296875, 0.29296875, 0.91796875, 0.41796875],
        [0.19140625, 0.69140625, 0.06640625, 0.56640625, 0.22265625, 0.72265625, 0.09765625, 0.59765625,
         0.19921875, 0.69921875, 0.07421875, 0.57421875, 0.23046875, 0.73046875, 0.10546875, 0.60546875],
        [0.94140625, 0.44140625, 0.81640625, 0.31640625, 0.97265625, 0.47265625, 0.84765625, 0.34765625,
         0.94921875, 0.44921875, 0.82421875, 0.32421875, 0.98046875, 0.48046875, 0.85546875, 0.35546875],
        [0.05078125, 0.55078125, 0.17578125, 0.67578125, 0.01953125, 0.51953125, 0.14453125, 0.64453125,
         0.05859375, 0.55859375, 0.18359375, 0.68359375, 0.02734375, 0.52734375, 0.15234375, 0.65234375],
        [0.80078125, 0.30078125, 0.92578125, 0.42578125, 0.76953125, 0.26953125, 0.89453125, 0.39453125,
         0.80859375, 0.30859375, 0.93359375, 0.43359375, 0.77734375, 0.27734375, 0.90234375, 0.40234375],
        [0.23828125, 0.73828125, 0.11328125, 0.61328125, 0.20703125, 0.70703125, 0.08203125, 0.58203125,
         0.24609375, 0.74609375, 0.12109375, 0.62109375, 0.21484375, 0.71484375, 0.08984375, 0.58984375],
        [0.98828125, 0.48828125, 0.86328125, 0.36328125, 0.95703125, 0.45703125, 0.83203125, 0.33203125,
         0.99609375, 0.49609375, 0.87109375, 0.37109375, 0.96484375, 0.46484375, 0.83984375, 0.33984375],
        [0.015625,   0.515625,   0.140625,   0.640625,   0.046875,   0.546875,   0.171875,   0.671875,
...
class ColorReducer:
    ...
    def reduce_colors(image: Image.Image, num_colors: int) -> List[Tuple[int, int, int]]:
        """Reduces the given image to 'num_colors' using median-cut."""
...
class ImageDitherer:
    ...
    def __init__(self,
                 ...
                 dither_params: Optional[Dict[str, Any]] = None):
    ...
    @staticmethod
    def get_mode_parameters(mode: DitherMode) -> Optional[Dict[str, Any]]:
        """Get parameter metadata for a specific dithering mode."""
        strategy_map = {
            DitherMode.BAYER: BayerDitherStrategy,
            DitherMode.HALFTONE: HalftoneDitherStrategy,
            DitherMode.POLKA_DOT: PolkaDotDitherStrategy,
            DitherMode.BLUE_NOISE: BlueNoiseDitherStrategy,
            DitherMode.INTERLEAVED_GRADIENT_NOISE: InterleavedGradientNoiseDitherStrategy,
            DitherMode.WAVELET: WaveletDitherStrategy,
            DitherMode.ADAPTIVE_VARIANCE: AdaptiveVarianceDitherStrategy,
            DitherMode.HYBRID: HybridDitherStrategy,
            DitherMode.ERROR_DIFFUSION: ErrorDiffusionDitherStrategy,
            DitherMode.OSTROMOUKHOV: OstromoukhovDitherStrategy,
        }
        strategy_class = strategy_map.get(mode)
        if strategy_class:
            return strategy_class.get_parameter_info()
        return None
    ...
    def _get_dither_strategy(self, mode: DitherMode) -> BaseDitherStrategy:
        """Create the appropriate dithering strategy for the given mode."""
        # Map of modes to strategy classes
        strategy_classes = {
            DitherMode.NONE: NoDitherStrategy,
            DitherMode.BAYER: BayerDitherStrategy,
            DitherMode.BLUE_NOISE: BlueNoiseDitherStrategy,
            DitherMode.INTERLEAVED_GRADIENT_NOISE: InterleavedGradientNoiseDitherStrategy,
            DitherMode.POLKA_DOT: PolkaDotDitherStrategy,
            DitherMode.ERROR_DIFFUSION: ErrorDiffusionDitherStrategy,
            DitherMode.RIEMERSMA: RiemersmaDitherStrategy,
            DitherMode.WAVELET: WaveletDitherStrategy,
            DitherMode.ADAPTIVE_VARIANCE: AdaptiveVarianceDitherStrategy,
            DitherMode.PERCEPTUAL: PerceptualDitherStrategy,
            DitherMode.HYBRID: HybridDitherStrategy,
            DitherMode.HALFTONE: HalftoneDitherStrategy,
            DitherMode.OSTROMOUKHOV: OstromoukhovDitherStrategy,
        }
        ...
        # Get parameter info and merge with user params
        param_info = strategy_class.get_parameter_info() if hasattr(strategy_class, 'get_parameter_info') else None
        
        if param_info:
            # Build settings from defaults + user overrides
            settings = {key: info['default'] for key, info in param_info.items()}
